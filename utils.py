...
def dice_coefficient(a, b):
    ...
    a = np.asarray(a).astype(np.bool_)
    b = np.asarray(b).astype(np.bool_)
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    with torch.no_grad():
        ...
        print("giong nhau a: ", (np.array_equal(label,prediction)))
        print("giong nhau a 2: ", (np.array_equal(result_label,result_pred)))
        # print(os.path.join('pred_image',filename))
        # plt.imsave(os.path.join('pred_image',filename)+'.png', result_pred/255, cmap='jet')
